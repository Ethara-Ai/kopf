...
class ActivityRegistry(GenericRegistry[handlers.ActivityHandler]):
    ...
    def iter_handlers(
        ...
        for handler in self._handlers:
            if (
                handler.activity is None
                or handler.activity == activity
        ...
        if not found:
            for handler in self._handlers:
                if (
                    handler.activity is None
                    or handler.activity == activity
                    and handler._fallback
                ):
                    yield handler


class ResourceRegistry(
    GenericRegistry[ResourceHandlerT], Generic[ResourceHandlerT, CauseT]
):
...
class IndexingRegistry(
    ResourceRegistry[handlers.IndexingHandler, causes.IndexingCause]
):
    def iter_handlers(
        self,
...
class WatchingRegistry(
    ResourceRegistry[handlers.WatchingHandler, causes.WatchingCause]
):
    def iter_handlers(
        self,
...
class SpawningRegistry(
    ResourceRegistry[handlers.SpawningHandler, causes.SpawningCause]
):
    def iter_handlers(
        self,
...
class ChangingRegistry(
    ResourceRegistry[handlers.ChangingHandler, causes.ChangingCause]
):
    def iter_handlers(
        self,
        ...
        for handler in self._handlers:
            if handler.id not in excluded:
                ...
                if matching_reason and matching_webhook:
                    ...
                    explicitly_for_deletion = set(handler.operations or []) == {
                        "DELETE"
                    }
...
def match(
    ...
    return (
        ...
        and _matches_subresource(handler, cause)
        and _matches_labels(handler, cause, kwargs)
        and _matches_annotations(handler, cause, kwargs)
        and _matches_field_values(handler, cause, kwargs)
        and _matches_field_changes(handler, cause, kwargs)
        and _matches_filter_callback(
            handler, cause, kwargs
        )  # the callback comes in the end!
    )
...
def _matches_resource(
    ...
    return handler.selector is None or handler.selector.check(resource)
...
def _matches_subresource(
    ...
    if not isinstance(cause, causes.WebhookCause):
        return True
    return (
        handler.subresource == "*" or handler.subresource == cause.subresource
    )  # incl. None==None case
...
def _matches_labels(
    ...
    return not handler.labels or _matches_metadata(
        pattern=handler.labels,
...
def _matches_annotations(
    ...
    return not handler.annotations or _matches_metadata(
        pattern=handler.annotations,
...
def _matches_metadata(
    *,
    ...
    content: Mapping[
        str, str
    ],  # from the body; can be live views on dicts (labels & co)
    kwargs: dict[str, Any],
    cause: causes.ResourceCause,
) -> bool:
...
def _matches_field_values(
    ...
    if isinstance(cause, causes.ChangingCause):
        ...
        values = [val]
    return (
        (handler.value is None and any(value is not absent for value in values))
        or (
            handler.value is filters.PRESENT
            and any(value is not absent for value in values)
        )
        or (
            handler.value is filters.ABSENT and any(value is absent for value in values)
        )
        or (
            callable(handler.value)
...
def _matches_field_changes(
    ...
    return (
        (
            not handler.field_needs_change or old != new  # ... or there IS a change.
        )
        and (
            (handler.old is None)
            or (handler.old is filters.ABSENT and old is absent)
            or (handler.old is filters.PRESENT and old is not absent)
            or (callable(handler.old) and handler.old(old, **kwargs))
            or (handler.old == old)
        )
        and (
            (handler.new is None)
            or (handler.new is filters.ABSENT and new is absent)
            or (handler.new is filters.PRESENT and new is not absent)
            or (callable(handler.new) and handler.new(new, **kwargs))
            or (handler.new == new)
        )
    )
